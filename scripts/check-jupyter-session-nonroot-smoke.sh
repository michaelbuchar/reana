...
wait_for_session_metadata() {
    for _ in $(seq 1 30); do
        session_metadata_json="$(client list --sessions -w "${WORKFLOW_NAME}" --verbose --json)"
...
SESSION_URI_PATH="$(python3 -c 'import sys; from urllib.parse import urlsplit; print(urlsplit(sys.argv[1]).path.rstrip("/") or "/")' "${SESSION_URI}")"
...
' "${POD_JSON_FILE}")"
SESSION_BASE_PATH="${SESSION_BASE_URL%/}"
[ -n "${SESSION_BASE_PATH}" ] || SESSION_BASE_PATH="/"
...
[ "${SESSION_URI_PATH}" = "${SESSION_BASE_PATH}" ] || {
...
home_probe="$(
    kubectl exec "${SESSION_POD_NAME}" -n "${NAMESPACE}" -- \
        python3 -c 'from pathlib import Path; probe = Path("/home/jovyan/reana-home-write-test.txt"); probe.write_text("ok"); value = probe.read_text(); probe.unlink(); print(value)'
)"
[ "${home_probe}" = "ok" ] || {
